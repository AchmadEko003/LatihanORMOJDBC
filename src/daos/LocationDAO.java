...
public class LocationDAO {
    ...
    public List<Location> searchData(Object object) {
        String query = "FROM LOCATION WHERE Location_Id LIKE '%" + object + "%'"
                + "OR Street_Address LIKE '%" + object + "%'"
                + "OR Postal_Code LIKE '%" + object + "%'"
                + "OR City LIKE '%" + object + "%'"
                + "OR State_Province LIKE '%" + object + "%'"
                + "OR Country_Id LIKE '%" + object + "%'";
        return data(query);
    }
    ...
    public Location getLocationId(Object id) {
        ...
        Transaction transaction = session.beginTransaction();
        try {
            location = (Location) session.createQuery("FROM LOCATION WHERE Location_Id = " + id).uniqueResult();
            transaction.commit();
        } catch (Exception e) {
            e.printStackTrace();
            if (transaction != null) {
